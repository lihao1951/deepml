...
class MeanShift(object):
    ...
    def plot_example(self):
        figure = plt.figure()
        ax = figure.add_subplot(111)
        ax.set_title("MeanShift Example")
        plt.xlabel("first dim")
        plt.ylabel("third dim")
        legends = []
        cxs = []
...
class SinglePass(object):
    ...
    def fit(self, X):
        ...
        for row in range(rows):
            ...
            final_group = None
            for group in self._groups:
                d = mydistance(xi, group.center, isEculidean=True)
                if d <= self._thresh:
                    final_group = group
    ...
    def plot_example(self):
        figure = plt.figure()
        ax = figure.add_subplot(111)
        ax.set_title("SinglePass Example")
        legends = []
        cxs = []
        cys = []
        for i in range(len(self._groups)):
